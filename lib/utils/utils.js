...
const REMOVE_KEYS = [
	'sync-settings.gistSettings.gistId',
	'sync-settings.gistSettings.personalAccessToken',
...
module.exports = {
	...
	async addExtraFile (files, file, removeUnfamiliarFiles) {
		...
		if (file === 'config.cson' && atom.config.get('sync-settings.gistSettings.personalAccessToken') && atom.config.get('sync-settings.hiddenSettings._warnBackupConfig')) {
