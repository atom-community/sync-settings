...
module.exports = class SyncSettings {
	...
	async checkForUpdate (autoCheck) {
		...
		let gistId
		try {
			const personalAccessToken = config.getPersonalAccessToken()
			if (!personalAccessToken) {
				notify.missingPersonalAccessToken(() => { this.checkForUpdate(autoCheck) })
				return
			}
			...
			const res = await githubApi.gists.get(personalAccessToken, { gist_id: gistId })
	...
	async backup () {
		...
		try {
			...
			const personalAccessToken = config.getPersonalAccessToken()
			if (!personalAccessToken) {
				notify.missingPersonalAccessToken(() => { this.backup() })
				return
			}
			...
			const res = await githubApi.gists.update(personalAccessToken, {
				gist_id: gistId,
...
[Click here to open your Gist.](${res.data.html_url})`.trim())
	...
	async restore () {
		...
		try {
			const personalAccessToken = config.getPersonalAccessToken()
			if (!personalAccessToken) {
				notify.missingPersonalAccessToken(() => { this.restore() })
				return
			}
	...
	async inputForkGistId () {
		const inputView = new InputView({ placeholder: 'Gist ID to Fork' })
	...
	async forkGistId (forkId) {
		...
		try {
			const personalAccessToken = config.getPersonalAccessToken()
			if (!personalAccessToken) {
				notify.missingPersonalAccessToken(() => { this.forkGistId(forkId) })
				return
			}

			const res = await githubApi.gists.fork(personalAccessToken, { gist_id: forkId })
