...
[Click here to open your Gist.](${res.data.html_url})`.trim())
	...
	async getLocalData () {
		...
		console.debug('sync-settings: Local Data', data)
		return data
	}
	...
	async restore () {
		...
		try {
			...
			const data = this.getBackupData(res.data.files)
			if (!data) {
				return
			}
	...
	getBackupData (files) {
		...
		console.debug('sync-settings: Backup Data', data)
		return data
	}
	...
	async getDiffData (localData, backupData) {
		const data = {
			settings: null,
			packages: null,
			files: null,
		...
		if (backupData.settings && localData.settings) {
			...
			if (Object.keys(settings).length > 0) {
				...
				if (settings.updated) {
					data.settings.updated = this.settingsToKeyPaths(settings.updated)
				}
				if (settings.deleted) {
		...
		if (backupData.packages && localData.packages) {
			...
			if (Object.keys(packages).length > 0) {
				...
				if (packages.deleted) {
					data.packages.deleted = packages.deleted
				}
			}
		...
		if (localData.files || backupData.files) {
			...
			for (const fileName of fileNames) {
				...
				const localFile = localData.files[fileName]
		...
		console.debug('sync-settings: Diff Data', data)
		return data
	}

	settingsToKeyPaths (obj, prefix = '') {
		const settings = []
		for (const prop in obj) {
			...
			if (typeof item === 'object' && !Array.isArray(item)) {
				settings.push(...this.settingsToKeyPaths(item, nextPrefix))
			} else {
				settings.push({
					keyPath: nextPrefix,
					value: item,
