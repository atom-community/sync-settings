		...
		if (localData.files || backupData.files) {
			...
			for (const fileName of fileNames) {
				...
				if (backupFile && localFile) {
					if (localFile.content.toString() !== backupFile.content.toString()) {
						const updated = {
							...backupFile,
							content: diff.createTwoFilesPatch('local', 'backup', localFile.content.toString(), backupFile.content.toString(), undefined, undefined, { context: 2 }),
						}
