...
module.exports = class DiffView {
	...
	async refresh () {
		...
		try {
			...
			diffData.localTime = config.getLastBackupTime(res, !diffData.settings && !diffData.packages && !diffData.files)
			diffData.backupTime = res.data.history[0].committed_at
		...
		if (!diff.settings && !diff.packages && !diff.files) {
			return (
				<div className='diff-view-none'>
					<h2>Your settings are all backed up. 😊</h2>
