...
Do you want to back up this file anyway?`.trim(),
		...
		const notification = this.error(invalidGistId ? 'Invalid Gist ID' : 'No Gist ID', {
			...
			buttons: [{
				...
				async onDidClick () {
					...
					let res
					try {
						res = await githubApi.gists.create(config.getPersonalAccessToken(), {
							public: false,
	...
	missingPersonalAccessToken (retryFn) {
		const notification = this.error('No Personal Access Token', {
			description: `
No Personal Access Token found in settings or \`GITHUB_TOKEN\`
