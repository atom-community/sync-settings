		...
		buttons: [{
			...
			async onDidClick () {
				...
				try {
					...
					if (atom.config.get('sync-settings.gistSettings.gistId')) {
						resolveFn(atom.config.get('sync-settings.gistSettings.gistId'))
					}
				} catch (ex) {}
				rejectFn()
			},
...
async function getGistId (allowEmpty) {
	let gistId = atom.config.get('sync-settings.gistSettings.gistId') || process.env.GIST_ID
	if (gistId) {
		return gistId.trim()
	}
...
and set the environment variable \`GITHUB_TOKEN\` or enter it in the settings.`.trim(),
		...
		buttons: [{
			...
			async onDidClick () {
				...
				if (personalAccessToken) {
					atom.config.set('sync-settings.gistSettings.personalAccessToken', personalAccessToken)
...
async function getPersonalAccessToken (required) {
	let token = atom.config.get('sync-settings.gistSettings.personalAccessToken') || process.env.GITHUB_TOKEN
	if (token) {
		return token.trim()
	}
...
module.exports = class Gist {
	...
	/**
	 * Defer loading until instantiated
	 */
	constructor () {
		if (!GitHubApi) {
	...
	async create () {
		...
		try {
			...
			const res = await createClient(personalAccessToken).gists.create({
				public: false,
			...
			atom.config.set('sync-settings.gistSettings.gistId', res.data.id)
	...
	async delete () {
		...
		try {
			personalAccessToken = await getPersonalAccessToken()
			gistId = await getGistId()
			await createClient(personalAccessToken).gists.delete({ gist_id: gistId })

			atom.config.unset('sync-settings.gistSettings.gistId')
	...
	async update (files) {
		...
		try {
			personalAccessToken = await getPersonalAccessToken()
			gistId = await getGistId()
			const res = await createClient(personalAccessToken).gists.update({
				gist_id: gistId,
				description: atom.config.get('sync-settings.gistSettings.gistDescription'),
				files,
			})
			...
			if (err) {
				return displayError(err, 'updating backup', () => this.update(files), gistId, personalAccessToken)
			}
		}
	}
	...
	async fork () {
		...
		try {
			...
			const res = await createClient(personalAccessToken).gists.fork({ gist_id: forkId })
			...
			if (err) {
				return displayError(err, 'updating backup', () => this.fork(), forkId, personalAccessToken)
			}
		}
	}
}
