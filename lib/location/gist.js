...
async function getGistId (allowEmpty) {
	let gistId = atom.config.get('sync-settings.gistId')
	if (gistId && !keychain.usingKeychain(gistId)) {
		return gistId.trim()
	}
...
and set the environment variable \`GITHUB_TOKEN\` or enter it in the settings.`.trim(),
		...
		buttons: [{
			...
			async onDidClick () {
				...
				if (personalAccessToken) {
					await setPersonalAccessToken(personalAccessToken)
...
async function getPersonalAccessToken (allowEmpty) {
	let token = atom.config.get('sync-settings.personalAccessToken')
	if (token && !keychain.usingKeychain(token)) {
		return token.trim()
	}
...
async function setPersonalAccessToken (token) {
	if (keychain.usingKeychain()) {
		try {
